...
- 前端：`/` 为管理员登录入口；`/admin` 为管理页（需管理员登录），内含社区账号登录、当前登录账户、**社区每日签到（一键签到）**、社区接口「接口演示」与任意接口 API 测试、退出登录
  ...
  - 🔒 **默认凭据一次性失效**：完成首次改密后（即已创建管理员账户），`admin` / `123456` **立即永久失效** —— 登录会被直接拒绝，且返回的提示与普通密码错误完全一致（不会暴露「默认凭据已禁用」这一状态）；也**不允许再改回**该默认组合（防止改回后自己被锁在门外）；登录页也不再显示初始账号密码提示。仅当系统仍处于未初始化状态（全新安装 / `admin.json` 被删除重建）时默认凭据才可用
